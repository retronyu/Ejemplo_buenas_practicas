...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_rap_travel_gp IN LOCAL MODE
       ENTITY Travel
  ...
  METHOD acceptTravel.
    " Establecer el nuevo estado general
    MODIFY ENTITIES OF zi_rap_travel_gp IN LOCAL MODE
    ENTITY travel
    UPDATE
    FIELDS ( TravelStatus )
    ...
    TravelStatus = travel_status-accepted
     ...
     FAILED failed
     REPORTED reported.
    " Rellenar la tabla de respuestas
    READ ENTITIES OF zi_rap_travel_gp IN LOCAL MODE
    ENTITY travel
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_rap_gp(
                                          ...
                                          textid   = zcm_rap_gp=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_gp(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_gp=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_gp(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_gp=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_gp(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_gp=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_GP( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_GP=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
...
* *                      %action-Edit         = COND #( WHEN update_granted = abap_true THEN if_abap_behv=>auth-allowed ELSE if_abap_behv=>auth-unauthorized )
  ...
  METHOD is_create_granted.
    ...
    AUTHORITY-CHECK OBJECT 'ZOSTATGP'
        ID 'ZOSTATGP' DUMMY
        ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATGP'
        ID 'ZOSTATGP' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATGP'
        ID 'ZOSTATGP' DUMMY
